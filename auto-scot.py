...
def build_hyper_model(hp):
    ...
    # Ajoutez une ou plusieurs couches LSTM en fonction de l'optimisation
    for i in range(hp.Int('num_lstm_layers', 1, 3)):
        model.add(LSTM(units=hp.Int(f'units_lstm_{i}', min_value=20, max_value=200, step=1), 
                       return_sequences=True))
        model.add(Dropout(rate=hp.Float(f'dropout_rate_lstm_{i}', min_value=0.0, max_value=0.5, step=0.1)))
