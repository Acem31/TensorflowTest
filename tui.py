...
def create_tui_window(stdscr):
    ...
    left_win.addstr(
        1, 2, "Appuyez sur F pour lancer le programme", curses.color_pair(2)
    )
    ...
    while True:
        ...
        if key == ord("F") or key == ord("f"):
            ...
            p = subprocess.Popen(
                ...
                stdout=slave,
                stderr=slave,
                preexec_fn=lambda: curses.resizeterm(max_y, curses.COLS // 3),
            )
            ...
            while True:
                try:
                    ...
                    if first_line:
                        right_win.addstr(
                            1, 2, " " * (curses.COLS // 3 - 4), curses.color_pair(2)
                        )
                        first_line = False
                    lines.append(output)
